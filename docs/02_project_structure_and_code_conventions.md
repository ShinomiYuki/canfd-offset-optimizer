...
`compare-weights` 在 `payload_bytes/` 与 `frame_time_us/` 子目录各保存一套完整比较
报告；根 `results/` 只保存两种模式各自的改善摘要和 GCLS Offset 差异，不跨单位排序。
所有 CSV/PNG 文件增加稳定的网段前缀，CSV 字段采用中文，但领域标识符（如
`CAN_ID`、`Offset`、`payload_bytes_GCLS_Offset`）保持原样。多网段目录根部生成
`ALL_offsets_weight_mode_comparison.csv`，提供逐网段、逐报文的交付总表。
