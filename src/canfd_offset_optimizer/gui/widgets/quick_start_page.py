    ...
    def __init__(self) -> None:
        ...
        layout.addWidget(
            self._group(
                ...
                "<b>网段速率参数</b>：Nominal Bitrate 只用于优化完成后的协议级保守占用诊断，"
                "不参与 GCLS；DBC 没有明确速率时保持待配置，不会猜测 500 kbit/s，也不会阻止优化。<br>"
        ...
        layout.addWidget(
            self._group(
                ...
                "原始和优化后使用同一批非路由周期 TX 报文；稳态默认重复显示"
                "当前网段自己的 4 个真实超周期。<br>"
                "<b>热力图</b>：上排是原始方案，下排是优化后方案；白、绿、黄、橙、红、黑表示"
                "0、1、2、3、4、5 帧及以上，只显示核心真实窗口，不重复。每格第三行是按协议上界"
                "估算的保守占用时间，不是当前权重或实测总线利用率；下方明细显示真实 Payload "
                "长度和每帧保守占用时间。<br>"
                "<b>网段速率参数</b>：Classic CAN 只需 Nominal；CAN FD 需要 Nominal、Data 和"
                "有效 BRS。DBC 有可靠 BRS 时按报文自动使用；缺失值可按网段确认。该配置只刷新"
                "结果诊断，不重新优化。<br>"
                "<b>显示范围</b>可选择 1、2、4 或 10 个完整超周期，只改变画面，"
                "不会重新运行优化，也不会修改核心结果。",
            )
        )
        layout.addWidget(
