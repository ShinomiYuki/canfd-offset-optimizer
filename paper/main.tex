...
因此，$H_D(\boldsymbol O)$ 足以表示当前软件侧精确子问题，可作为缓存键，并支持类型压缩和后续可达软件代价水平诊断。该表示并非 $P^\star$ 的一一编码：不同直方图仍可能得到相同的最优软件代理成本。若模型加入报文特异执行成本、兼容性或容量约束，报文身份将重新影响组成本，直方图表示也未必继续充分。软件代价水平数与二维非支配目标向量数的组合上界见推论~\ref{cor:pstar-pareto-cardinality}。
...
四个 Offset assignment、全部两报文分组及完整目标像的枚举见附录第~\ref{subsec:appendix-two-message-conflict-proof}~节。
...
再从经典 \textsc{Partition} 问题归约\cite{karp1972}。给定正整数 $a_1,\ldots,a_n$。若其总和为奇数，则输出一个固定 no-instance：取两条权重分别为 1 和 2 的报文，每条均可选择只命中两个时隙之一的候选，并令阈值为 1。总负载为 3，无论如何分配，至少一个时隙的负载不小于 2，故该实例必为否。
...
该证明由 \textsc{Partition} 得到的是受限子类的弱数值困难性，不支持 strong NP-hardness，也不覆盖由合法 \texttt{frame\_time\_us} 产生的物理权重子类或隐式超周期表示。它同样不排除伪多项式、近似、参数化或实际规模上的精确方法。定理的作用是刻画显式外层选择的最坏情形，而不是推出“只能采用启发式”。
...
该结论依赖所有组共享公共正参数 $\rho$、变量成本对报文数线性且不区分身份，并且不存在禁止同组、容量、节点归属或其他兼容约束。它只保证存在一个结构化最优解，不排除另有拆分相同 D 类型的等成本最优解。
...
\begin{equation}
 O\!\left(3^m\operatorname{poly}(|I|)\right).
\end{equation}
该形式符合参数 $m$ 下 $f(m)|I|^{O(1)}$ 的 FPT 定义\cite{downey2013,mnich2015}。它不是关于报文数 $n$ 的一般多项式算法；当每条报文都有不同 D 类型时，$m=n$，指数项仍可达到 $3^n$。规范锚点递推采用的是既有集合划分思想\cite{bjorklund2009}，本文的结构依据是 same-D 交换性质允许把报文级实例无损压缩到类型级实例。
...
\begin{equation}
 O\!\left(m2^m\operatorname{poly}(|I|)\right).
\end{equation}
两者分别对应抽象值/前驱表示和当前对象表示，不能混为同一个空间结论。规范消歧只在成本完全相等时选择稳定代表，不改变上述最优值证明。
...
综上，当前结论依赖公共 $\rho$、身份对称线性成本和无额外兼容约束。它说明 D 直方图足以确定当前 $P^\star$，但不说明不同直方图必对应不同成本，也不能用于压缩身份相关的扩展模型。

\subsection{软件代价水平与二维非支配目标点数上界}
\label{subsec:appendix-structure-bounds}

令 $n$ 为报文数，$\mathcal R=\bigcup_i\mathcal D_i$，$r=|\mathcal R|$，并令 $\mathcal H_{\mathrm{reachable}}$ 为合法 Offset 分配实际能够诱导的 D 直方图集合。以下推论只计数不同软件代价值和不同二维目标向量，不计数产生同一结构或目标的 assignment 数量。
...
最后固定任一软件水平 $P^\star=p$。在所有具有该第二维坐标的不同目标向量中，只有 $Q^{\mathrm{ss}}$ 最小者可能非支配；同层更大的 $Q^{\mathrm{ss}}$ 被它支配。因此每个软件水平至多贡献一个不同的非支配目标向量，得到式~\eqref{eq:appendix-pareto-level-bound}。Peak 约束若存在，只会缩小可行 assignment 集，不改变这一计数论证。
\end{proof}

该推论提供的是结构性上界，不保证达到等号，也不能把目标向量数解释为 Offset 分配数。特别地，多个 assignment 可以共享同一目标向量，而不同可达直方图也可以共享同一软件代价水平。

\subsection{两报文严格冲突构造的完整枚举}
\label{subsec:appendix-two-message-conflict-proof}

本节补全命题~\ref{prop:two-message-strict-conflict}。证明在正文式~\eqref{eq:two-message-conflict-instance} 的实例上枚举四个 Offset assignment 及全部可能的两报文分组，然后直接比较完整目标像。
...
\begin{table}[htbp]
\centering
\caption{两报文实例的四个 Offset assignment 与完整目标值}
...
两报文只有合并和两个单例两种无序划分，表中已经全部列出。对于 $(20,20)$，合并成本比两个单例少 $\rho/20>0$，故 $P^\star=(\rho+2)/20$；对 $(25,25)$ 同理有 $P^\star=(\rho+2)/5$。两个混合 assignment 只交换报文身份，在当前身份对称成本下产生相同目标向量，其 $P^\star$ 为 $(\rho+2)/5$ 与 $(\rho+1)/4$ 的较小者。
...
\end{equation}
故 $P_A^\star<P_B^\star$；同时 $Q_B^{\mathrm{ss}}=2w^2<4w^2=Q_A^{\mathrm{ss}}$。因此 A 与 B 在两个最小化目标上严格不可比。$(25,25)$ 与 A 具有相同的 $Q^{\mathrm{ss}}$，但软件成本严格更高，故受 A 支配。两个混合 assignment 又映射到同一个 B。由此，完整目标像恰有 A、B 两个非支配目标点。
\end{proof}
