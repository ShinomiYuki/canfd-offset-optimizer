...
\noindent\hspace*{\parindent}车载周期报文的 Offset 既决定总线时间轴上的释放相位，也会影响 ECU 软件侧周期入口的调用结构。针对通信侧局部释放均衡与软件周期调度代价通常分开处理的问题，本文提出一种面向周期 CAN/CAN FD 报文的 Offset 协同优化方法。通信侧在统一离散时间轴上建立带权释放负载模型，以稳态峰值和负载平方和分别评价局部聚集与整体均衡。在有限候选及稳态时隙命中列表均显式给出的条件下，受限峰值判定问题为 NP-complete；外层据此采用 GCLS 启发式方法搜索相位配置。

在共同整数时间原点与精确周期触发假设下，本文建立 Offset 与软件调用周期的耦合关系，并以 $D_i=\gcd(T_i,O_i)$ 刻画单报文最大可行调用周期。固定 Offset 后，相同 D 类型能够无损压缩，软件分组可由关于不同 D 类型数 $m$ 的确定性子集动态规划精确求解。通信均衡与软件代理成本可能严格冲突，因此本文采用 Peak 约束下的 $\varepsilon$-constraint 联合搜索，并以观测 Pareto 前沿和几何膝点组织不同软件预算下的折中候选。

九个真实 CAN FD 网段的实验中，Peak 模式相较原始配置在六个网段降低了稳态峰值。对合法 Offset 空间诱导的软件结构进行完整枚举后，九个网段均存在多个可达软件代价水平；当前联合归档中，六个网段形成多点观测 Pareto 前沿，EP、LC 与 SU 形成单点观测前沿。固定 Offset 内层由精确算法求解，联合外层及其前沿仍是有限搜索预算下的观测结果。本文方法已实现为面向实际工程输入的优化工具并投入使用，实际应用中观察到周期释放热点改善。
...
\noindent\hspace*{\parindent}The Offset of a periodic in-vehicle message determines its release phase on the bus timeline and also affects the invocation structure of periodic entry functions on the ECU. This thesis therefore studies joint Offset optimization for periodic CAN/CAN FD messages rather than treating communication balancing and software scheduling cost separately. A weighted release-load model on a unified discrete timeline uses the steady-state peak and the sum of squared loads to evaluate local clustering and overall balance. With finite candidates and steady-state slot-hit lists given explicitly, the restricted peak decision problem is NP-complete; a GCLS heuristic is used for the outer Offset search.

Under a common integer time origin and exact periodic triggering, $D_i=\gcd(T_i,O_i)$ gives the maximum feasible invocation period of one message. For a fixed Offset assignment, messages with the same D type can be compressed without loss, and deterministic subset dynamic programming solves the software grouping problem exactly with the number $m$ of distinct D types as the parameter. Because communication balance and the software cost proxy can conflict strictly, a Peak-constrained $\varepsilon$-constraint search is used to organize trade-off candidates into observed Pareto fronts and geometric knee points.

Experiments on nine real CAN FD networks show that Peak mode reduces the steady-state peak on six networks relative to the original configurations. Complete enumeration of the software structures induced by legal Offsets finds multiple reachable software-cost levels on every network. In the current joint archives, six networks exhibit multi-point observed Pareto fronts, whereas EP, LC, and SU yield single-point observed fronts. The fixed-Offset inner problem is solved exactly, while the joint outer search and its fronts remain observations under finite search budgets. The method has been implemented as an engineering optimization tool, and its practical use has shown improvements in periodic-release hotspots.
...
这一核心问题同时涉及通信相位、软件周期组织和多目标取舍，现有研究则分别从这些方向展开。CAN/CAN FD 周期报文的 Offset 与相位配置首先服务于通信时序分析。经典响应时间分析从固定优先级非抢占传输中的阻塞与高优先级干扰出发，建立报文最坏响应时间和可调度性判据\cite{tindell1994}；在此基础上，CAN Offset 配置与 Offset-aware 响应时间分析表明，利用相位关系使周期流去同步，能够改善部分高负载场景的时序表现\cite{grenier2008,yomsi2012}。这些工作确立了 Offset 作为通信配置自由度的意义。本文不以离散释放负载替代完整的响应时间或可调度性分析，而是在有限 Offset 候选上评价各稳态时隙中的带权释放聚集。
...
固定 Offset 后的软件分组属于集合划分问题。通用集合划分的指数时间精确算法已有系统研究\cite{bjorklund2009}，调度问题中按不同数值或类型数量进行参数化求解也已有成熟背景\cite{mnich2015}；因此，子集动态规划和按类型数参数化的思想本身并非本文的新颖点。对于通信与软件两个不同量纲的目标，Pareto 支配、$\varepsilon$-constraint 与膝点同样是成熟的多目标分析工具\cite{mavrotas2009,debgupta2011}。本文关注的是同一 Offset 在两侧的共同作用：它既进入显式稳态时隙的带权释放评价，也进入不区分报文身份的 MainFunction 代理成本。固定 Offset 后的软件分组由精确内层求解，外层则只报告实际搜索到的 Offset 候选；后者与能够完整分析的 D 结构采用不同证据口径。

\subsection{研究方法与主要贡献}

上述研究分别处理通信相位、软件周期组织与多目标分析。本文在其基础上聚焦同一 Offset 对两侧结构的共同影响，主要工作如下。
...
\begin{enumerate}
  ...
  \item 固定完整 Offset 后，利用当前 gcd 型且不区分报文身份的组成本，把相同 D 类型的报文压缩为带重数的类型集合划分，并以确定性子集动态规划精确求解。这里利用的是特定成本下的结构保持性质，而不是把通用集合划分递推作为新方法。
  ...
  \item 在固定 Offset 精确内层之上构造受 Peak 约束的 $\varepsilon$-constraint CAN--CPU 联合搜索，并以观测 Pareto 前沿组织候选；同时区分可完整枚举的 D 结构与有限外层搜索实际访问的联合候选。
...
\end{equation}
集合 $\mathcal F_{i,d}$ 正式称为候选域上的 gcd 纤维，即产生同一 D 值的 Offset 候选集合；它不应一般地理解为一个普通同余类。相应的软件侧 D 等价关系定义为
\begin{equation}
 o\sim_i o'
...
\end{equation}
同一 D 等价类中的 Offset 对报文 $i$ 的最大可行调用周期具有相同影响，但它们在 CAN 时间轴上的释放位置仍可能不同。因此，软件侧可以按 D 等价类压缩，通信侧仍须评价具体 Offset。下一章先在完整候选空间上定义通信侧目标，随后再利用这一差异构造“外层选择 Offset、固定 Offset 后精确处理软件分组”的求解层次。
...
\begin{proof}[\normalfont 证明思路]
给定一个 Offset 分配后，只需扫描所选候选的显式命中列表，累加各时隙负载并与 $\Theta$ 比较，因此证书可在显式输入长度的多项式时间内验证。困难性由经典 PARTITION 问题归约\cite{karp1972}：把每个正整数 $a_i$ 作为一条报文的权重，并为该报文设置分别只命中两个时隙之一的两个 Offset 候选。若总权重为 $2B$，取阈值 $\Theta=B$，则两个时隙的负载都不超过 $B$ 当且仅当原整数集合存在等和划分。该构造已经满足定理所列的同周期、二候选、二时隙和单次命中限制；判定版本的 NP-completeness 随即推出对应最小化问题的 NP-hardness。完整的 NP 成员资格、奇数总和处理、双向归约与编码长度证明见附录第~\ref{subsec:appendix-explicit-peak-proof}~节。
\end{proof}

该归约只给出受限子类的弱数值困难性，不证明 strong NP-hardness，也不覆盖由合法 CAN FD \texttt{frame\_time\_us} 产生的物理权重子类。NP 成员资格依赖命中列表显式给出；若周期与超周期采用紧凑数值隐式编码，展开验证未必对输入位长为多项式\cite{zeng2006periodic,short2010periodic}。这些边界仍允许伪多项式、近似、参数化及小规模精确方法；定理刻画的是外层组合选择的最坏情形，而不是“只能采用启发式”。
...
第\ref{subsec:explicit-offset-peak-complexity}节说明了外层 Offset 组合选择在最坏情形下为何困难，却没有规定唯一的求解方法。候选较少时，完整枚举、CP-SAT 和分支定界仍可提供精确结果或可验证的对照；实际网段则需要在更大的候选域和有限时间预算之间取舍。
...
以下压缩与精确性结论共同采用本章的既定模型前提：全部时间量位于共同整数栅格，所有分组共享同一个正参数 $\rho$，组成本为线性且不区分报文身份，即不依赖报文名称、CAN ID 或其他身份信息；可行划分不含禁止同组、容量、节点归属或报文特异执行成本等附加约束，$P^\star$ 采用精确有理数比较。在这些条件下，对任意非空组 $G$，其代理成本可写为
\begin{equation}
 c(G)
 =
 \frac{\rho+|G|}
...
\end{equation}
式\eqref{eq:fixed-offset-group-cost}表明，一个分组的评价只依赖其中的 $D_i$ 取值、相同取值的出现次数及 $\rho$；具体报文身份只在类型级结果映射回报文时使用。
...
\begin{proof}[\normalfont 证明思路]
若同一 D 类型分散在两个组中，记两组的 TimeBase 满足 $B_1\le B_2$，并将第一组内的一条该类型报文移入第二组。接收组的最大公约数不变；若移出后第一组仍非空，其最大公约数保持不变或增大，线性报文数项的变化使总成本不增加。若第一组原为单例，则 $B_1=B_2$，合并还会省去一次固定调用成本。有限次执行这一操作，即可从任一最优方案得到一个不拆分相同 D 类型的最优方案。完整交换证明见附录第~\ref{subsec:appendix-same-d-compression-proof}~节。
\end{proof}

命题\ref{prop:same-d-compression}保证至少存在一个不拆分相同 D 类型的最优解，而不是断言所有最优解都具有这一结构。类型压缩据此把组合维度从 $n$ 条报文降为 $m$ 个不同 D 类型，并以重数 $c_j$ 保留各类型的报文数量。类型集合仍需划分，但外层 Offset 对软件最优值的影响已经可以通过其诱导的 D 结构传递。
...
\begin{proof}[\normalfont 证明思路]
D 向量相同时，每个报文级划分中各组的最大公约数、组大小和成本均保持不变。D 直方图相同时，两个实例具有相同的类型值及重数；由命题\ref{prop:same-d-compression}，两者都可无损压缩为同一个类型级划分问题，故精确最优值相同。保持 D 类型的报文双射证明、可达约数集合及必要反例见附录第~\ref{subsec:appendix-d-structure-proof}~节。
\end{proof}

因此，$H_D(\boldsymbol O)$ 可以表示当前软件侧精确子问题，并用于缓存和可达软件代价水平诊断。不同直方图仍可能得到相同的 $P^\star$；若模型加入报文特异执行成本、兼容性或容量约束，直方图也可能不再足以确定最优成本。相应的软件代价水平数与二维非支配目标向量数上界见推论~\ref{cor:pstar-pareto-cardinality}。确定这一压缩后，下一节只需在 D 类型集合上求解剩余的划分问题。
...
\begin{proof}[\normalfont 证明思路]
命题\ref{prop:same-d-compression}保证至少存在一个报文级全局最优方案可以表示为 D 类型划分。引理\ref{lem:anchor-block-uniqueness}保证锚点递推对类型划分不重不漏，最优子结构则使每个状态由一个锚点组与余集最优状态组成。由集合大小归纳即可得到 $F(\mathcal U)$ 的全局最小性。完整归纳证明见附录第~\ref{subsubsec:appendix-anchor-proof}~节。
\end{proof}
...
式\eqref{eq:two-message-q-conflict}与式\eqref{eq:two-message-pstar-conflict}给出命题\ref{prop:two-message-strict-conflict}的见证，说明两目标在一般模型中不具有天然的单调一致性。方案 A 与 B 的稳态 Peak 分别为 $2w$ 和 $w$；Peak 硬约束同时接纳二者时冲突保留，预算位于 $[w,2w)$ 时方案 A 被排除，该冲突则可从可行域中消失。这一见证说明联合评价并非冗余，但不表示每个预算或真实网段都必然产生多点前沿。

四种 Offset 分配、全部两报文分组及完整目标像的枚举见附录第~\ref{subsec:appendix-two-message-conflict-proof}~节。

单独最小化 $Q^{\mathrm{ss}}$ 可能提高 $P^\star$，只优化软件成本也可能保留通信热点。下一节因此先用 Peak 硬约束限定可接受的通信峰值，再以 $\varepsilon$-constraint 分析两者折中；外层仍是启发式候选搜索，所得前沿只覆盖已访问的候选。
...
三个 CP-SAT 运行均以 \texttt{FEASIBLE} 而非 \texttt{OPTIMAL} 状态结束。表中结果证明存在优于相应 GCLS 解的可行分配，但既不是经证明的 CP-SAT 最优值，也不能用于计算 GCLS 相对于真实全局最优值的精确差距；求解日志中的下界与当前最佳可行解之间仍存在间隙。RQ3 因而支持对这三个 GCLS 解全局最优性的否定，但不形成所有网段上两类方法的统一性能排序。
...
命题~\ref{prop:pstar-d-structure} 已经证明，当组成本不区分报文身份时，$P^\star(\boldsymbol O)$ 只依赖 Offset 所诱导的 $D$ 结构。基于这一性质，本节不枚举完整的 Offset 笛卡尔积，而是在 $D$ 直方图层面精确回答每个真实网段能够形成多少种软件侧结构，以及这些结构对应多少个不同的 $P^\star$ 水平。该问题与联合 Pareto 前沿是否完整无关：它只刻画软件侧的完整可达结构，不同时穷举每个 Offset 分配的 $Q^{\mathrm{ss}}$。
...
完整枚举中的“可达”表示至少存在一个合法 Offset 分配能够诱导相应直方图或软件成本，但不保留产生它的全部分配数量。由于当前组成本不区分报文身份，这足以完整回答软件侧结构问题；通信侧的 $Q^{\mathrm{ss}}$ 仍取决于具体报文及其释放相位，不能由同一计数恢复。因而，RQ5 的完整性严格限定在 $D/P^\star$ 层面，不扩展为完整 Offset 分配覆盖或联合目标覆盖。
...
完整结构枚举说明软件侧存在多个可达水平，但只有同时评价具体 Offset 分配的 $Q^{\mathrm{ss}}$，才能判断哪些二维组合非支配。联合外层尚未穷举全部 Offset 分配或完整的 $Q^{\mathrm{ss}}$--$P^\star$ 目标像；下一节因此转向有限预算联合搜索实际观察到的二维前沿。
...
PT 的推荐不属于几何膝点，三个单点网段也没有可供解释的前沿曲率。其余五个几何膝点只是当前观测前沿及既定归一化、弦距规则下的折中候选，并非唯一工程最优。九网段均得到满足既定约束的观测解，其中六个网段呈现多点权衡；推荐是否稳定还取决于模型参数和外层搜索配置，下一节对此进行检验。
...
这一变化不只是预算网格分辨率的提高。外层求解采用启发式搜索，改变 $\varepsilon$ 预算点还会改变实际访问的约束搜索路径、在位解与热启动的演化，以及候选归档能够发现的分配方案。因此，当前 $K$ 敏感性同时包含网格采样效应和启发式搜索路径效应，不能据此把 $K=41$ 的结果解释为全局 Pareto 前沿。

当前联合结果能够展示已访问候选中的 $Q^{\mathrm{ss}}$--$P^\star$ 权衡，但具体推荐仍依赖 CPU 开销代理模型参数与有限启发式搜索条件。下一章据此把完整软件结构、精确内层和观测联合前沿分开讨论。
...
外层 Offset 一旦固定，$D_i=\gcd(T_i,O_i)$ 就确定单报文最大可行调用周期，组内 $D_i$ 的最大公约数进一步确定 MainFunction 的 TimeBase。由于当前组成本不区分报文身份，命题~\ref{prop:same-d-compression} 保证至少存在一个不拆分相同 D 类型的最优方案；类型级锚点递推因而能够精确覆盖所需划分。该算法的指数部分只依赖不同 D 类型数 $m$，九个真实网段的 $m$ 为 4--6。运行时间说明这些实例可处理，但不代替关于 $m$ 的参数化复杂度证明。

对于外层已访问的每个 $\boldsymbol O$，求解器给出的 $P^\star(\boldsymbol O)$ 是当前代理模型下的最小分组成本。定理~\ref{thm:fixed-offset-dp-exactness} 给出数学依据，141/141 组报文级穷举则交叉核验当前实现。RQ5 进一步枚举全部可达 D 直方图并精确计算对应的 $P^\star$，说明九网段均具有多个离散软件代价水平。固定 Offset 评价和软件结构枚举由此保持精确，搜索不确定性集中在联合外层；有限枚举本身不替代定理证明。
...
RQ5 的完整枚举覆盖所有合法 Offset 所能诱导的 D 直方图，并据此得到全部可达 $P^\star$ 水平。它没有逐一穷举 Offset 分配，也没有建立完整的 $Q^{\mathrm{ss}}$--$P^\star$ 目标像：多个分配可以共享同一 D 直方图和软件成本，却产生不同的通信释放结构与 $Q^{\mathrm{ss}}$。
...
软件侧能够避开重复分配，是因为当前成本不区分承载同一 D 值的具体报文；通信侧却不能作相同压缩。帧时间权重、释放相位和报文身份共同决定时隙负载，同一直方图下的不同分配仍可能产生不同 $Q^{\mathrm{ss}}$。软件结构完整与联合目标完整之间缺少的不是内层求解精度，而是对具体 Offset 分配及其通信结果的完整覆盖。

RQ6 的观测 Pareto 前沿只从 GCLS、有限 $\varepsilon$ 网格和迭代细化访问到的累积候选中提取。DA 达到轮数上限时仅目标签名稳定，DK、GL 和 IC 的目标与分配签名均未稳定；RQ7 也显示种子和网格会改变候选覆盖。相同目标点还可能对应不同 Offset 分配，因此目标稳定不表示配置唯一，达到轮数上限也只表示预算耗尽。完整 $D/P^\star$ 结构不能把这些观测结果升级为完整 Pareto 前沿；准确口径仍是“固定 Offset 内层精确、联合外层启发式”。

\subsection{单点与多点结果如何解释}
\label{subsec:discussion-single-multi}

CH、DA、DK、GL、IC 和 PT 的当前联合归档形成多点观测前沿，说明已访问候选中存在离散取舍。EP、LC 和 SU 经二维支配筛选后各保留一个点，但完整枚举分别得到 91、114 和 114 个可达 $P^\star$ 水平，因此三个单点结果不能解释为软件目标在全空间恒定。

这种取舍来自同一 Offset 的两条作用路径：$O_i$ 在通信侧改变释放相位和时隙负载，在软件侧则与 $T_i$ 共同确定 $D_i$，并经组内最大公约数影响 TimeBase 与调用频率。第~\ref{subsec:joint-strict-conflict} 节的两报文构造证明两种偏好可能严格冲突，但不证明每个网段或每个 Peak 预算下都有多点前沿。

单点现象可能由 Peak 约束、二维支配和有限搜索覆盖共同形成，现有证据不能分离三者；多个可达 $P^\star$ 水平也不保证形成多个非支配点。多点网段可从当前归档的端点和内部候选中选择，单点网段则返回唯一观测解而不称几何膝点。几何膝点、理想点回退和唯一观测解都只为归档内审阅提供顺序，不是唯一全局最优配置。

\subsection{模型与工程边界}
\label{subsec:discussion-boundaries}

本文以帧时间权重评价周期释放热点，并以固定调用成本与逐报文增量成本的比例比较软件周期组织。正式联合实验中的 $\rho=1$ 是未标定基准；目标 ECU 仍需通过性能剖析估计 $C_0$、$C_1$ 及其比例。该代理保留 MainFunction 数量、组内报文数和最大可行 TimeBase 等结构关系，但不等同于实际 CPU 利用率，也不包含调度抢占、缓存行为和异质执行路径。若标定后的成本依赖报文身份、处理路径或兼容约束，$P^\star$ 的数值解释、最优分组和 D 直方图压缩都需重新确认。
...
工程工具已经把 CAN-only 与 CAN--CPU Joint 纳入同一工作流。前者完成输入资格筛选、Offset 优化、结果审阅与 DBC Offset 输出，后者在相同输入语义上执行联合搜索，展示观测 Pareto 前沿并生成 MainFunctionTx 分组和 TimeBase 建议。核心算法返回的同一推荐分配同时驱动 Offset 表、负载图、DBC replacement、MainFunction 分组和联合导出；GUI 不在显示层重新计算或替换推荐。若核心算法没有返回合法结果，工具只保留诊断信息，不伪造分配、软件建议或 DBC 写回。
...
当前机器可读产物能够复核输入资格、候选合法性、Peak 约束、二维非支配性、MainFunction 覆盖关系和导出候选，九网段结构枚举还给出了软件侧全部可达水平。下一步通信验证应引入优先级、控制器队列、背景流量和错误恢复，检验热点降低能否转化为响应时间或超时风险的改善；软件验证则需测量不同 MainFunction 数量、组规模和 TimeBase 的执行成本，并在真实任务配置下考虑抢占、缓存和共享资源。

当前工具已经给出 Joint 推荐及 MainFunctionTx、TimeBase 建议，但尚未自动生成目标项目可直接使用的 AUTOSAR 配置。后续需将推荐映射到 ARXML 与 ECU 构建流程，完成 $\rho$ 标定，再开展 CAN 与任务时序分析、硬件在环和整车回归。因此，本文完成的是从工程输入到配置候选的工作链；从候选到可执行配置及系统级保证仍属于后续工作。
...
理论方面，受物理 \texttt{frame\_time\_us} 权重限制的 Offset 峰值问题复杂性仍待研究；外层还需要更强的精确算法、近似保证或参数化结果，以提供可验证的解质量与覆盖界。对于 EP、LC 和 SU 的单点观测，还需通过受控对照分别量化 Peak 约束、二维支配关系和搜索覆盖的作用，而不能把任一因素预设为唯一原因。

模型方面，需要在目标 ECU 上通过性能剖析估计 $C_0$、$C_1$ 与 $\rho$，并结合实际执行时间和系统时序分析校准软件代理。进一步扩展到报文特异执行成本和分组兼容约束时，现有相同 D 类型压缩与直方图表示需要重新检验其适用性。
...
正文式\eqref{eq:anchor-transition-count}给出规范锚点动态规划的转移总数。该计数、精确有理数运算的位复杂度、FPT 结论以及值/前驱表示与当前实现的两种空间上界，统一在附录第~\ref{subsec:appendix-dp-proof}~节证明。此处仅保留单位换算与代理成本的代数推导，避免在两个附录位置重复同一复杂度论证。
...
本附录给出第~\ref{sec:offset-balancing-model}~章至第~\ref{sec:can-cpu-joint}~章中主要定理、命题、推论与引理的完整证明。证明所用可行域、比较向量、CPU 开销代理指标和分组成本均沿用正文定义，不引入新的优化模型；其中“精确”始终只限定于固定完整 Offset 分配下的 MainFunction 分组子问题。代理成本的代数展开与单位换算仍置于附录第~\ref{subsec:appendix-mainfunction-derivations}~节，本附录则集中给出复杂性、结构性质和计数上界的论证。

\subsection{显式带权离散 Offset 峰值判定的 NP 完全性}
\label{subsec:appendix-explicit-peak-proof}

本节补全定理~\ref{thm:explicit-offset-peak-complexity}。证明路线是先给出与正文式~\eqref{eq:explicit-offset-peak-decision}一致的显式输入，再证明证书可在该输入长度的多项式时间内验证，最后由 \textsc{Partition} 构造受限的二候选、二时隙实例。

一个显式带权离散 Offset 峰值判定实例由报文集合、各报文的正整数权重 $w_i$、有限候选集 $\Oset_i$、每个候选显式给出的稳态时隙命中列表 $\Hset_{i,o}^{\mathrm{ss}}$，以及非负整数阈值 $\Theta$ 构成。列表允许同一时隙重复出现，以表示同一稳态窗口内的多次释放。问题询问是否存在完整分配 $\boldsymbol O\in\prod_i\Oset_i$，使所有稳态时隙负载均不超过 $\Theta$。

\begin{proof}[定理~\ref{thm:explicit-offset-peak-complexity} 的完整证明]
先证该问题属于 NP。给定完整 Offset 分配作为证书，验证器逐条检查 $O_i\in\Oset_i$，扫描所选候选的显式命中列表，并按出现次数把 $w_i$ 累加到相应时隙，最后将每个累计负载与 $\Theta$ 比较。扫描次数不超过所选列表的总长度；累计值的位长至多在权重最大位长上增加关于报文数和列表长度的对数量级。因此，候选合法性、负载累计和阈值比较都能在显式输入长度的多项式时间内完成。这里的 NP 成员资格依赖命中列表显式给出；若只用二进制周期隐式描述可能指数长的超周期展开，上述验证时间结论不能直接沿用。

再从经典 \textsc{Partition} 问题归约\cite{karp1972}。给定正整数 $a_1,\ldots,a_n$。若其总和为奇数，则输出如下固定 no-instance：取两条权重分别为 1 和 2 的报文，令二者周期均为 $10\,\mathrm{ms}$、Offset 候选均为 $\{15,20\}\,\mathrm{ms}$，并采用 $5\,\mathrm{ms}$ 时隙、稳态窗口 $[20,30)\,\mathrm{ms}$ 和阈值 1。每个候选恰在两个时隙之一命中一次，总负载为 3，故任意分配都使至少一个时隙的负载不小于 2。该固定实例不仅为否，也满足定理~\ref{thm:explicit-offset-peak-complexity} 所列的同周期、二候选、二时隙和单次命中限制。

若总和为偶数 $2B$，对每个 $a_i$ 构造一条权重 $w_i=a_i$ 的报文，并令
\begin{equation}
 \Delta=5\,\mathrm{ms},\qquad
 T_i=10\,\mathrm{ms},\qquad
 \Oset_i=\{15,20\}\,\mathrm{ms}.
\label{eq:appendix-partition-construction}
\end{equation}
取稳态窗口 $[20,30)\,\mathrm{ms}$ 和阈值 $\Theta=B$。候选 $20\,\mathrm{ms}$ 在窗口内于 $20\,\mathrm{ms}$ 释放，命中第一个时隙；候选 $15\,\mathrm{ms}$ 的下一次释放位于 $25\,\mathrm{ms}$，命中第二个时隙。因周期均为 $10\,\mathrm{ms}$，每条报文在该窗口内恰释放一次，所以每个 $a_i$ 恰被分配到两个时隙之一。

若原实例存在和为 $B$ 的子集，就令该子集选择 $20\,\mathrm{ms}$，其余元素选择 $15\,\mathrm{ms}$。两个时隙的负载都为 $B$，从而 $Z^{\mathrm{ss}}\le B$。反之，若构造实例存在满足 $Z^{\mathrm{ss}}\le B$ 的 Offset 分配，由于两个时隙的总负载固定为 $2B$，且每个时隙均不超过 $B$，两者只能都等于 $B$。选择 $20\,\mathrm{ms}$ 的报文权重因而构成原实例的等和子集。故 $Z^{\mathrm{ss}}\le B$ 当且仅当原 \textsc{Partition} 实例可等分。

该构造产生 $O(n)$ 个候选及显式命中项；除复制 $a_i$ 与 $B$ 外，所有时间数值均为常数，输出数值的二进制编码长度关于原输入长度为多项式。因此这是多项式 many-one 归约。结合 NP 成员资格，判定问题即使限制为同周期、每条报文两个候选、两个稳态时隙且每候选只命中一次，仍为 NP-complete；相应峰值最小化问题为 NP-hard。
\end{proof}

该证明由 \textsc{Partition} 得到的是受限子类的弱数值困难性，不支持 strong NP-hardness，也不覆盖由合法 \texttt{frame\_time\_us} 产生的物理权重子类或隐式超周期表示。它不排除伪多项式、近似、参数化或实际规模上的精确方法。外层困难性明确后，下一节转向固定 Offset 内层能够利用的 D 类型结构。

\subsection{same-D 类型压缩与最优性保持}
\label{subsec:appendix-same-d-compression-proof}

本节补全命题~\ref{prop:same-d-compression} 的交换证明。目标不是证明每个最优划分都不拆分相同 D 类型，而是从任一全局最优划分出发，通过有限次不增成本的移动得到至少一个具有该结构的最优划分。

\begin{proof}[命题~\ref{prop:same-d-compression} 的完整证明]
设同一类型 $d$ 分散在两个组 $G_1$ 与 $G_2$ 中，其调用周期分别为 $B_1$ 与 $B_2$。两者都整除 $d$，不妨令 $B_1\le B_2$，并从 $G_1$ 向 $G_2$ 移动一条类型 $d$ 的报文。

若移动后 $G_1$ 仍包含类型 $d$，则两组的最大公约数均不变，固定组开销也不变。报文数变量项的总变化为
\begin{equation}
 -\frac{1}{B_1}+\frac{1}{B_2}\le 0,
\label{eq:appendix-same-d-case-one}
\end{equation}
故总成本不增加。

若移出的是 $G_1$ 中最后一条类型 $d$ 的报文，但余组 $H=G_1\setminus\{i\}$ 仍非空，记 $B'_1=\gcd_{j\in H}D_j$。从最大公约数的参数集合中删除 $d$ 后，$B'_1$ 是 $B_1$ 的倍数，因此 $B'_1\ge B_1$。令 $|H|=h$，并令移动前 $|G_2|=n_2$。接收组原已含有类型 $d$，故加入同类型报文不改变 $B_2$；两个相关组的成本变化满足
\begin{equation}
 \begin{aligned}
 &\left(
   \frac{\rho+h}{B'_1}
 ...
 \le0.
 \end{aligned}
\label{eq:appendix-same-d-case-two}
\end{equation}
第一个不等式使用了 $B'_1\ge B_1$ 以及分子为正这一条件。

若 $G_1$ 原为被移动报文的单例，则 $B_1=d$。又因 $B_2\mid d$ 且 $B_1\le B_2$，只能有 $B_2=d$。移动后原组消失，变量成本在两个组之间抵消，并省去原单例组的固定项，故总成本变化为 $-\rho/d<0$。

三种情形均不增加成本。对一个分散类型反复移动，有限次后该类型集中到一个组；依次处理全部分散类型不会重新拆分已经集中的类型。由类型和报文数均有限，过程终止。若初始划分全局最优，所得划分的成本不可能低于全局最优值，又不高于初始成本，因而仍为全局最优。
\end{proof}

该结论依赖所有组共享公共正参数 $\rho$、变量成本对报文数线性且不区分身份，并且不存在禁止同组、容量、节点归属或其他兼容约束。它只保证存在一个结构化最优解，不排除另有拆分相同 D 类型的等成本最优解；这一存在性正是下一节把报文级划分压缩为类型级动态规划的依据。

\subsection{类型级子集动态规划的精确性与参数化复杂度}
\label{subsec:appendix-dp-proof}

本节证明定理~\ref{thm:fixed-offset-dp-exactness}。证明先说明规范锚点递推对无序类型划分不重不漏，再由最优子结构和归纳得到精确性，最后推导转移数量、位复杂度和两种空间上界。

\subsubsection{锚点递推与精确性}
\label{subsubsec:appendix-anchor-proof}

令 $\mathcal U=\{1,\ldots,m\}$ 为 D 类型索引全集，$F(S)$、$a(S)$、$N(A)$、$B(A)$ 与 $C(A)$ 沿用式~\eqref{eq:subset-message-count}--\eqref{eq:anchor-subset-dp}。任一非空 $S$ 的无序划分中，各组两两不交且并集为 $S$，因此恰有一个组包含锚点 $a(S)$。记该组为 $A$，则余下各组唯一构成 $S\setminus A$ 的无序划分；反之，把任一满足 $a(S)\in A\subseteq S$ 的集合 $A$ 与余集的任一无序划分合并，也唯一得到 $S$ 的一个无序划分。故枚举所有包含锚点的 $A$ 对无序划分不重不漏，这同时补全引理~\ref{lem:anchor-block-uniqueness}。

\begin{proof}[定理~\ref{thm:fixed-offset-dp-exactness} 的精确性部分]
对 $|S|$ 归纳。基础情形 $S=\varnothing$ 只有空划分，成本为 $F(\varnothing)=0$。假设所有大小小于 $k$ 的状态都取得相应类型集合上的最小划分成本，现取 $|S|=k$。任一 $S$ 的划分都有唯一锚点组 $A$；固定该组后，若余集不采用 $F(S\setminus A)$ 对应的最优划分，就可用更低成本的余集划分替换并改进原方案。因此，以 $A$ 为锚点组的最小总成本恰为 $C(A)+F(S\setminus A)$。再对全部含锚点的 $A$ 取最小值，因其不重不漏覆盖所有划分，式~\eqref{eq:anchor-subset-dp} 即得到 $S$ 的真实最小值。

于是 $F(\mathcal U)$ 是类型级全局最优值。由命题~\ref{prop:same-d-compression}，至少存在一个报文级全局最优划分不拆分任何 D 类型，因而可表示为 $\mathcal U$ 的类型划分；任一类型划分又可展开成对应的报文级划分。两层可行解的这一包含关系说明类型级最优值等于报文级最优值，故动态规划精确得到固定 Offset 下的 $P^\star$。
\end{proof}

\subsubsection{转移数量、位复杂度与空间}
\label{subsubsec:appendix-dp-complexity-derivation}

对大小为 $k$ 的非空状态 $S$，锚点固定后其余 $k-1$ 个类型可自由选择是否进入当前组，故该状态有 $2^{k-1}$ 个候选转移。按状态大小求和并使用二项式定理，得到
\begin{equation}
 \begin{aligned}
 \sum_{\varnothing\ne S\subseteq[m]}2^{|S|-1}
 &=\sum_{k=1}^{m}\binom{m}{k}2^{k-1}\\
 &=\frac{1}{2}\left(\sum_{k=0}^{m}\binom{m}{k}2^k-1\right)\\
 &=\frac{(1+2)^m-1}{2}
 =\frac{3^m-1}{2}.
 \end{aligned}
\label{eq:subset-enumeration-count}
\end{equation}
这证明正文式~\eqref{eq:anchor-transition-count} 的精确计数，而不是只给出渐近结论。

下面补足数值运算的位复杂度。设输入整数最大位长为 $L$，并将正有理数 $\rho=p/q$ 的分子、分母编码计入 $|I|$。子集 gcd 可用 Euclid 算法计算，其位复杂度关于 $L$ 为多项式；重数和至多为报文数 $n$，位长为 $O(\log n)$。单组成本可写成
\begin{equation}
 C(A)=\frac{p+qN(A)}{qB(A)},
\label{eq:appendix-rational-group-cost}
\end{equation}
其分子、分母位长均为 $\operatorname{poly}(|I|)$。一个 DP 值至多相加 $m$ 个组成本；即使按分母乘积作保守估计，中间分子和分母的位长也只累加至多 $m$ 个输入级位长，仍为输入编码长度的多项式。分数加法、交叉乘比较、最大公约数约分，以及按前驱重建至多 $m$ 个组，因而都只贡献 $\operatorname{poly}(|I|)$ 因子。

预计算全部 $2^m$ 个子集统计量需要 $O(2^m\operatorname{poly}(|I|))$ 时间，加上式~\eqref{eq:subset-enumeration-count} 的转移，得到
\begin{equation}
 O\!\left(3^m\operatorname{poly}(|I|)\right).
\end{equation}
该形式符合参数 $m$ 下 $f(m)|I|^{O(1)}$ 的 FPT 定义\cite{downey2013,mnich2015}。它不是关于报文数 $n$ 的一般多项式算法；当每条报文都有不同 D 类型时，$m=n$，指数项仍可达到 $3^n$。规范锚点递推采用的是既有集合划分思想\cite{bjorklund2009}，本文的结构依据是相同 D 类型的交换性质允许把报文级实例无损压缩到类型级实例。

空间上，若每个状态只保存一个最优有理数值与一个前驱 mask，共有 $2^m$ 个状态，所需空间为
\begin{equation}
 O\!\left(2^m\operatorname{poly}(|I|)\right).
\end{equation}
当前实现还为每个状态保存规范分组结构并预存子集签名；每个结构最坏含 $O(m)$ 个类型索引，故实现口径的上界为
\begin{equation}
 O\!\left(m2^m\operatorname{poly}(|I|)\right).
\end{equation}
两者分别对应抽象值/前驱表示和当前对象表示，不能混为同一个空间结论。规范消歧只在成本完全相等时选择稳定代表，不改变上述最优值证明。动态规划的输入仅由 D 类型及其重数构成，下一节进一步说明 Offset 如何通过这一结构决定软件最优成本。

\subsection{软件代理成本对 D 结构的依赖}
\label{subsec:appendix-d-structure-proof}

本节补全命题~\ref{prop:pstar-d-structure}。证明依次刻画单报文可达 D 集合、D 向量和 D 直方图，并用两个反例说明直方图充分性既不是一一对应，也不自动延伸到身份相关成本。

由式~\eqref{eq:reachable-d-set}，
\begin{equation}
 \mathcal D_i=\{\gcd(T_i,o):o\in\Oset_i\}.
\end{equation}
每个最大公约数都整除 $T_i$，因此
\begin{equation}
 \mathcal D_i\subseteq\operatorname{Div}(T_i).
\label{eq:appendix-reachable-divisor-subset}
\end{equation}
该包含通常为真包含，且可达集合未必构成约数格的子格。例如 $T_i=12$、$\Oset_i=\{2,3\}$ 时，$\mathcal D_i=\{2,3\}$；其 gcd 为 1、lcm 为 6，均不属于该集合。故候选 Offset 所诱导的是可达约数子集，而不是 $T_i$ 的全部约数。

\begin{proof}[命题~\ref{prop:pstar-d-structure} 的完整证明]
先设两个 Offset 分配 $\boldsymbol O$ 与 $\boldsymbol O'$ 产生相同 D 向量。对任意固定报文分组 $\Pi$，每个对应组含有相同的 $D_i$，所以组的 $B_G$、报文数和式~\eqref{eq:fixed-offset-group-cost} 给出的成本完全相同。两边允许的报文集合划分也相同，故每个 $\Pi$ 的总成本逐项相等；在同一划分集合上取最小值即得 $P^\star(\boldsymbol O)=P^\star(\boldsymbol O')$。

再设两分配只具有相同 D 直方图。对每个 D 值，把 $\boldsymbol O$ 下具有该值的报文与 $\boldsymbol O'$ 下具有该值的报文任取一一对应，由相同重数可把这些对应合并为保持 D 类型的报文双射。该双射把一侧任一报文划分逐组映射为另一侧唯一划分，并保持每组的 D 多重集、最大公约数、报文数和成本；其逆映射同样成立。因此两侧可行划分及其逐组成本一一对应，最小总成本相同。
\end{proof}

直方图充分不意味着它与软件代价水平一一对应。取 $\rho=1$，D 向量 $(10,10)$ 的合并成本为 $3/10$；D 向量 $(10,20)$ 的合并成本也是 $3/10$，两个单例的总成本同样为 $2/10+2/20=3/10$。两种直方图不同，却有相同的 $P^\star=3/10$。

组成本不区分报文身份这一条件也不可删除。考虑扩展组成本
\begin{equation}
 \frac{\rho+\sum_{i\in G}c_i}{\gcd_{i\in G}D_i},
\label{eq:appendix-identity-specific-cost}
\end{equation}
令 $\rho=1$，两个报文的特异变量成本分别为 1 和 10。实例一的 D 向量为 $(20,5)$，实例二把 D 值对换为 $(5,20)$；两者直方图相同。逐一比较合并与两个单例后，前者最优成本为 $23/10$，后者为 $19/20$。因此，一旦成本依赖报文身份，相同 D 直方图也可能产生不同最优值。

当前结论依赖公共 $\rho$、不区分报文身份的线性成本和无额外兼容约束。D 直方图足以确定当前 $P^\star$，但不同直方图仍可能对应同一成本，身份相关的扩展模型也不能沿用这一压缩。下一节据此只对可达软件代价水平和不同二维目标向量计数。

\subsection{软件代价水平与二维非支配目标点数上界}
\label{subsec:appendix-structure-bounds}

令 $n$ 为报文数，$\mathcal R=\bigcup_i\mathcal D_i$，$r=|\mathcal R|$，并令 $\mathcal H_{\mathrm{reachable}}$ 为合法 Offset 分配实际能够诱导的 D 直方图集合。以下推论只计数不同软件代价值和不同二维目标向量，不计数产生同一结构或目标的分配数量。

\begin{corollary}[软件代价水平与二维目标前沿的组合上界]
\label{cor:pstar-pareto-cardinality}
在命题~\ref{prop:pstar-d-structure} 的模型前提下，
\begin{equation}
 |\operatorname{range}(P^\star)|
 \le\prod_i|\mathcal D_i|
 \le\prod_i\tau(T_i),
\label{eq:appendix-pstar-vector-bound}
\end{equation}
且
\begin{equation}
 |\operatorname{range}(P^\star)|
 \le|\mathcal H_{\mathrm{reachable}}|
 \le\min\left\{
  \prod_i|\mathcal D_i|,
  \binom{n+r-1}{r-1}
 \right\}.
\label{eq:appendix-pstar-histogram-bound}
\end{equation}
对不同二维目标向量构成的非支配前沿，还有
\begin{equation}
 |\mathrm{PF}_{\rm objective}|
 \le|\operatorname{range}(P^\star)|.
\label{eq:appendix-pareto-level-bound}
\end{equation}
\end{corollary}

\begin{proof}
每个 Offset 分配先映射到 $\prod_i\mathcal D_i$ 中的一个 D 向量，再映射到一个 $P^\star$ 值，故软件代价水平数不超过 D 向量数 $\prod_i|\mathcal D_i|$。由式~\eqref{eq:appendix-reachable-divisor-subset}，$|\mathcal D_i|\le\tau(T_i)$，得到式~\eqref{eq:appendix-pstar-vector-bound}。

命题~\ref{prop:pstar-d-structure} 又说明每个可达直方图至多对应一个 $P^\star$ 值，因此代价水平数不超过 $|\mathcal H_{\mathrm{reachable}}|$。可达直方图由可达 D 向量诱导，所以其数量不超过 $\prod_i|\mathcal D_i|$。另一方面，若暂时忽略每条报文各自的可达集合限制，只计数 $r$ 个 D 值上的非负整数重数且总和为 $n$ 的直方图，stars-and-bars 给出 $\binom{n+r-1}{r-1}$ 个可能向量。实际可达直方图只是其中的子集，故式~\eqref{eq:appendix-pstar-histogram-bound} 成立；该组合数不是“每个直方图均可达”的断言。

最后固定任一软件水平 $P^\star=p$。在所有具有该第二维坐标的不同目标向量中，只有 $Q^{\mathrm{ss}}$ 最小者可能非支配；同层更大的 $Q^{\mathrm{ss}}$ 被它支配。因此每个软件水平至多贡献一个不同的非支配目标向量，得到式~\eqref{eq:appendix-pareto-level-bound}。Peak 约束若存在，只会缩小可行分配集合，不改变这一计数论证。
\end{proof}

该推论提供的是结构性上界，不保证达到等号，也不能把目标向量数解释为 Offset 分配数。多个分配可以共享同一目标向量，不同可达直方图也可以共享同一软件代价水平。

\subsection{两报文严格冲突构造的完整枚举}
\label{subsec:appendix-two-message-conflict-proof}

本节补全命题~\ref{prop:two-message-strict-conflict}。证明在正文式~\eqref{eq:two-message-conflict-instance} 的实例上枚举四种 Offset 分配及全部可能的两报文分组，然后直接比较完整目标像。

取 $T_1=T_2=20\,\mathrm{ms}$、$\Oset_1=\Oset_2=\{20,25\}\,\mathrm{ms}$、$w_1=w_2=w>0$、$\rho>0$，稳态窗口为 $[25,45)\,\mathrm{ms}$，时隙宽度为 $5\,\mathrm{ms}$。Offset 20 在窗口内于 $40\,\mathrm{ms}$ 释放，Offset 25 于 $25\,\mathrm{ms}$ 释放，每条报文恰释放一次。表~\ref{tab:appendix-two-message-enumeration} 使用毫秒倒数的无公共尺度成本；生产值只相差不改变排序的正比例因子。

\begin{table}[htbp]
\centering
\caption{两报文实例的四种 Offset 分配与完整目标值}
\label{tab:appendix-two-message-enumeration}
\scriptsize
\setlength{\tabcolsep}{3.2pt}
\begin{tabular}{@{}lclllll@{}}
\toprule
$\boldsymbol O$ & 释放关系 & $Q^{\mathrm{ss}}$ & D 向量 & 合并成本 & 两单例成本 & $P^\star$ \\
\midrule
$(20,20)$ & 同槽 & $4w^2$ & $(20,20)$ & $(\rho+2)/20$ & $2(\rho+1)/20$ & $(\rho+2)/20$ \\
$(20,25)$ & 异槽 & $2w^2$ & $(20,5)$ & $(\rho+2)/5$ & $(\rho+1)/4$ & 两者较小值 \\
$(25,20)$ & 异槽 & $2w^2$ & $(5,20)$ & $(\rho+2)/5$ & $(\rho+1)/4$ & 两者较小值 \\
$(25,25)$ & 同槽 & $4w^2$ & $(5,5)$ & $(\rho+2)/5$ & $2(\rho+1)/5$ & $(\rho+2)/5$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{proof}[命题~\ref{prop:two-message-strict-conflict} 的完整证明]
两报文只有合并和两个单例两种无序划分，表中已经全部列出。对于 $(20,20)$，合并成本比两个单例少 $\rho/20>0$，故 $P^\star=(\rho+2)/20$；对 $(25,25)$ 同理有 $P^\star=(\rho+2)/5$。两个混合分配只交换报文身份，在当前不区分报文身份的成本下产生相同目标向量，其 $P^\star$ 为 $(\rho+2)/5$ 与 $(\rho+1)/4$ 的较小者。

记 A 为 $(20,20)$，B 为任一混合方案。对任意 $\rho>0$，
\begin{equation}
 \frac{\rho+2}{20}<\frac{\rho+2}{5},
 \qquad
 \frac{\rho+2}{20}<\frac{\rho+1}{4},
\label{eq:appendix-two-message-pstar-order}
\end{equation}
故 $P_A^\star<P_B^\star$；同时 $Q_B^{\mathrm{ss}}=2w^2<4w^2=Q_A^{\mathrm{ss}}$。因此 A 与 B 在两个最小化目标上严格不可比。$(25,25)$ 与 A 具有相同的 $Q^{\mathrm{ss}}$，但软件成本严格更高，故受 A 支配。两个混合分配又映射到同一个 B。由此，完整目标像恰有 A、B 两个非支配目标点。
\end{proof}

A 与 B 的 Peak 分别为 $2w$ 与 $w$。上述两点前沿结论适用于没有 Peak 约束，或 Peak 预算同时接纳两者的情形；若预算位于 $[w,2w)$，A 被排除，严格权衡可从可行域中消失。因此该构造只证明两个目标一般不天然同向，不证明每个真实网段、每个预算或每次有限外层搜索都必然产生多点前沿。

\subsection{有限枚举与实现交叉核验的证据位置}
\label{subsec:appendix-theory-verification}

正文 RQ4 报告的 141/141 组报文级 Bell 枚举，用于核对类型级精确求解器返回的最小成本；冻结理论审计中的 V1--V5 则以确定案例和固定种子随机案例检查 PARTITION 构造、两报文冲突、D 直方图边界、类型 DP 与结构计数上界。这些有限枚举能够发现构造、实现或边界反例，并与本附录的形式推导相互核对，但它们不是数学证明，也不替代上述 NP 归约、交换论证、归纳证明和计数证明。141/141 的证据强度因此始终限定为实现交叉核验。
...
\subsubsection{固定 Offset 下的内层最优替换性质}
...
第~\ref{subsubsec:gcls-balanced-pool}~节说明的 3-opt 只在冲突导向筛选后的有限三报文集合内枚举候选，且默认关闭。为隔离其作用，表~\ref{tab:appendix-three-opt} 比较同一单候选入口在关闭和启用该扩展时的结果。
