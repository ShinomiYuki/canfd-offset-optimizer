...
第~\ref{sec:gcls-method}~章已经给出一个具体的离散 Offset 分配方案。相位确定后，研究重点由总线侧转向软件周期入口：这些报文的周期与释放相位能否通过周期调用准确表达？当多条报文共享一个 MainFunction 时，还需要确定共同采用的调用周期以及报文之间的分组方式。因此，给定完整 Offset 分配后，软件侧仍有两个核心未知量，即各 MainFunction 能够采用的共同调用周期和报文集合的分组方案。
...
共同时间基准与精确对齐是本文生成周期调用建议时采用的建模条件；AUTOSAR COM 的 MainFunction 与 TimeBase 配置语义及基础软件的周期组织可参见文献\cite{autosarcom,zhang2011autosar,denil2011devs}，具体部署仍需结合目标平台的实际调度实现。式\eqref{eq:exact-tick-alignment}将单报文的可表示性转化为 $B$ 对 $T_i$ 与 $O_i$ 的共同整除关系，单报文问题因而进一步归结为在这些公共正约数中确定可采用的最大调用周期。
...
代理模型明确后，固定 $\boldsymbol O$ 下的软件侧问题转化为：在所有 MainFunction 划分中寻找 $P_{\rm CPU}$ 最小的方案。此时，真正的困难不再是成本如何定义，而是报文级集合划分数量会随问题规模迅速增长。

\subsection{D 类型压缩与最优性保持}
\label{subsec:d-type-compression}

对 $n$ 条报文直接枚举 MainFunction 划分，候选数量按 Bell 数增长。固定 $\boldsymbol O$ 后，每条报文的 $D_i$ 已由式\eqref{eq:message-max-tick}确定，分组调用周期由组内 $D_i$ 的最大公约数决定，而代理成本进一步只依赖各 D 类型、相应报文数量及 $\rho$。因此，在进入精确枚举前需要回答：具有相同 $D_i$ 的多条报文，是否仍有必要作为彼此不同的个体参与划分？对任意非空组 $G$，其代理成本可写为
\begin{equation}
 c(G)
 =
 \frac{\rho+|G|}
...
\begin{proof}[\normalfont 证明思路]
若同一 D 类型分散在两个组中，可将 TimeBase 较小组内的一条同类型报文移入另一组。该移动不改变接收组的最大公约数；移出组的最大公约数保持不变或增大，因而总成本不会上升。反复执行交换即可由任一最优方案得到一个不拆分相同 D 类型的最优方案。完整交换证明见附录第~\ref{subsubsec:appendix-same-d-proof}~节。
\end{proof}

命题\ref{prop:same-d-compression}只保证至少存在一个具有该结构的最优解，并不声称所有等成本最优解都必须满足这一性质。这意味着，将报文级划分压缩为 D 类型级划分不会排除全部全局最优方案；其意义不只是减少求解状态，而是在保留至少一个全局最优方案的前提下，将决策对象从 $n$ 条具体报文转化为 $m$ 个不同 D 类型，各类型的重数 $c_j$ 仍完整保留相应报文数量。类型集合本身仍存在多种划分，余下的问题是如何不重不漏地枚举这些分组，并利用最优子结构精确求得最小成本。
...
\begin{theorem}[固定 Offset 分组子问题的精确性]
\label{thm:fixed-offset-dp-exactness}
给定一个完整且固定的 Offset 分配方案，若全部时间量位于共同整数栅格，组成本采用式\eqref{eq:fixed-offset-group-cost}，参数 $\rho>0$，且式\eqref{eq:anchor-subset-dp}完整枚举所有包含规范锚点的类型子集，则 D 类型压缩与子集动态规划返回本章 CPU 开销代理模型内的全局最小 MainFunction 分组成本。
\end{theorem}
...
定理\ref{thm:fixed-offset-dp-exactness}的全局最优性限定于给定 $\boldsymbol O$ 后的 MainFunction 分组子问题。

这种精确性以 D 类型集合上的指数枚举为代价。子集动态规划包含 $O(2^m)$ 个状态，候选分组的组合枚举使核心时间规模为 $O(3^m)$；其中 $m$ 表示不同 D 类型的数量，而不是具体报文数量。实际实现还包含关于 $m$ 的多项式额外开销，完整组合计数推导见附录第~\ref{subsubsec:appendix-dp-complexity-derivation}~节。

实现采用整数时间和精确有理数比较代理成本，并以规范消歧规则在等成本最优方案中选择唯一代表；这些规则只保证结果稳定，不改变式\eqref{eq:cpu-proxy}的目标值及其最小成本。

因此，对任一已经确定的完整 Offset 分配方案，本章方法都能精确返回模型内的最小软件调用开销代理值及一个规范最优分组。外层评价 Offset 时无需再同时启发式搜索 MainFunction 划分，这一结果为通信与软件目标的联合优化提供了结构基础。
\clearpage
\section{CAN--CPU 联合优化}
\label{sec:can-cpu-joint}

一个完整 Offset 分配方案 $\boldsymbol O$ 一旦确定，第~\ref{sec:offset-balancing-model}~章定义的通信侧评价即可给出其局部释放指标，第~\ref{sec:main-function-exact}~章的精确求解器则可返回该分配方案下软件分组的最小 CPU 开销代理指标。因而，同一外层决策不再只对应 CAN 侧的均衡质量，而是同时产生通信侧与软件侧两类评价结果。本章关注的问题是：如何在 Peak 不发生不可接受退化的条件下，搜索稳态负载平方和 $Q^{\mathrm{ss}}$ 与软件侧最小代理成本之间的折中。
...
若把 Offset 分配 $\boldsymbol O$ 与 MainFunction 划分 $\Pi$ 同时作为外层变量，联合搜索空间将叠加相位分配与集合划分两类组合复杂度。第~\ref{sec:main-function-exact}~章给出的结构性质使这一联合枚举没有必要：对任意 $\boldsymbol O\in\Omega_{\rm joint}$，记 $\mathcal P(\boldsymbol O)$ 为相应的可行划分集合，则固定 Offset 下的最小代理成本为
\begin{equation}
 P^\star(\boldsymbol O)
 =
...
\end{equation}
由定理\ref{thm:fixed-offset-dp-exactness}，该最小值可由 D 类型压缩与确定性子集动态规划精确取得。下文以 $\Pi^\star(\boldsymbol O)$ 表示精确求解器按既定消歧规则返回的一个最优划分。
...
本章从三个层面验证前述方法：考察 GCLS 对九个真实网段局部释放分布的改善，验证固定 Offset 下 MainFunction 精确子问题的实现正确性与计算规模，并分析联合优化所得 $Q^{\mathrm{ss}}$--$P^\star$ 权衡及其稳定性。九网段结果构成主要实验，开发阶段的诊断实验则用于解释启发式搜索及参数变化所呈现的算法行为。
...
\begin{enumerate}
 ...
 \item \textbf{RQ5：}九网段联合优化得到的观测 Pareto 前沿、细化状态和后处理推荐具有怎样的结构？
 \item \textbf{RQ6：}随机种子、$\rho$、$\varepsilon$ 网格与搜索路径对联合结果有何影响？
\end{enumerate}
...
\subsection{RQ5：九网段 CAN--CPU 联合优化结果}
\label{subsec:exp-rq5}

\subsubsection{观测端点与观测 Pareto 前沿}
\label{subsubsec:exp-rq5-fronts}
...
\label{subsubsec:exp-rq5-refinement}
...
第~\ref{sec:experiments} 节呈现了两个相互关联的现象：CAN-only Offset 优化在六个网段降低了稳态峰值，而 CH、EP 和 SU 未观察到进一步改善；联合优化则在六个网段形成多点观测 Pareto 前沿，在 EP、LC 和 SU 仅保留单一非支配点。这些差异共同表明，Offset 的价值不仅取决于相位能否移动，还取决于周期、候选相位、报文权重与软件周期组织之间的结构关系。因而，实验结果的主要意义在于揭示不同网段为何呈现不同收益、通信与软件目标为何可能一致或冲突，以及这些认识在何种工程条件下成立。

\subsection{Offset 相位重排的收益与网段依赖性}
\label{subsec:discussion-offset}

Offset 提供的是相位重排自由度，能够形成何种局部释放结构则由具体实例共同决定。周期集合及其倍数关系决定释放序列在统一评价窗口中的重复方式，合法候选 Offset 和原始相位限定可到达的位置，报文数量与帧时间权重决定不同重叠的代价，离散时间刻又将这些因素映射为有限的局部聚集模式。因此，即使采用相同的搜索方法，不同网段也不应预期获得相同幅度的 $Z^{\mathrm{ss}}$ 或 $Q^{\mathrm{ss}}$ 改善。

Peak 在六个网段取得较低稳态峰值，说明真实工程配置中确实存在可利用的相位重排空间；CH、EP 和 SU 的等值结果则说明该空间的可利用程度具有实例依赖性。在当前候选域与搜索预算内，这三个网段未出现更低的 Peak，但该结果并不构成原始配置全局不可改进的证明。由此更合适的认识是，Offset 均衡并非稳定产生固定比例收益的通用变换，而是利用具体周期集合中已有的相位自由度重新组织局部释放。

Peak 控制最高的局部聚集，$Q^{\mathrm{ss}}$ 则描述整个评价窗口内的负载形状，二者不存在一般的单调对应关系。主实验中 Balanced 与 Peak 在九个网段得到相同的核心 CAN 指标，表明在 $5\%$ Peak 容差、当前候选池和搜索预算下，尚未发现满足峰值约束而进一步改善 $Q^{\mathrm{ss}}$ 的方案。第~\ref{subsubsec:exp-rq2-tolerance-pool} 节的容差与候选池诊断产生了不同结果，说明这种一致性属于当前实例与配置，而非两类目标在数学上的等价。Variance 在部分网段以更高峰值换取更低 $Q^{\mathrm{ss}}$ 的结果也印证了两项指标的互补性。相位一旦同时约束软件侧调用周期，“更有利的相位”便进一步分化为通信侧与软件侧两种可能不同的结构偏好。

\subsection{CAN--CPU 权衡的结构来源}
\label{subsec:discussion-can-cpu-tradeoff}

同一 Offset $O_i$ 沿两条路径影响联合评价。在通信侧，它改变周期报文的释放相位以及时隙负载分布，进而作用于 $Z^{\mathrm{ss}}$ 和 $Q^{\mathrm{ss}}$；在软件侧，$(T_i,O_i)$ 决定单报文最大可行调用周期 $D_i$，各 $D_i$ 的组合又通过 $B_G$ 决定 MainFunction 的 TimeBase 与调用频率。CAN 侧通常受益于高权重释放在时间轴上的分散，而软件侧则倾向于形成较有利的最大公约数关系，使更多报文能够共享较大的 $B_G$，从而减少高频固定调用开销。

这两种偏好之间没有一般的单调映射。使 CAN 释放更分散的 Offset 可能降低部分 $D_i$，迫使相关 MainFunction 采用更短的调用周期；有利于形成较大公共 TimeBase 的 Offset 也可能使报文在 CAN 时间轴上重新聚集。该模型结构解释了 CAN 均衡与软件周期成本为何可能形成权衡，也为第~\ref{subsec:exp-rq5} 节观察到的 $Q^{\mathrm{ss}}$--$P^\star$ 关系提供了解释框架。由于现有实验没有逐报文隔离并检验每一条最大公约数路径，这一机制用于解释权衡的可能来源，而不被表述为某个具体网段的因果证明。

CH、DA、DK、GL、IC 和 PT 的多点观测 Pareto 前沿表明，在当前累积候选中，没有一个方案能够同时替代全部其他保留方案，通信均衡与软件开销代理之间存在实际可观察的取舍。EP、LC 和 SU 的单点结果则表示当前候选归档中没有观察到第二个值得保留的二维非支配方案；它们没有显示多点权衡结构，并不排除扩大搜索后出现新方案。权衡是否显现由此同样具有网段依赖性，而不是联合模型对所有实例必然产生的固定形态。

几何膝点和理想点回退只在已经形成的观测前沿上提供解释性浏览与推荐顺序，不把连续的工程偏好压缩为唯一最优配置。联合坐标中的 $Q^{\mathrm{ss}}$ 可由给定 Offset 直接评价，而 $P^\star(\boldsymbol O)$ 来自固定 Offset 下的 MainFunction 内层问题。由此需要进一步说明：在外层仍为启发式搜索时，精确内层为联合结论提供了何种确定性。

\subsection{精确内层与启发式外层的分层意义}
\label{subsec:discussion-exact-heuristic}

精确内层不会使整个 CAN--CPU 联合问题转化为精确优化，但它消除了固定 Offset 下 MainFunction 分组近似所带来的不确定性。对于外层已经访问到的每个 $\boldsymbol O$，第~\ref{sec:main-function-exact} 节的求解器给出的 $P^\star(\boldsymbol O)$ 是当前 CPU 开销代理模型下可达到的最小分组成本。因此，若某个观测候选的软件侧成本较高，其原因不再是 MainFunction 可能尚未充分分组，而是该 Offset 通过 $D_i$ 与 $B_G$ 的结构限制了模型内最低代理成本。精确内层由此把主要搜索覆盖不确定性集中到外层 Offset 选择，使联合候选之间的软件侧比较具有一致基准。

定理~\ref{thm:fixed-offset-dp-exactness} 从最优子结构和枚举完备性给出固定 Offset 子问题的精确性依据；141 组独立穷举对照均得到相同成本，则从实现层面交叉验证了程序与模型递推的一致性。两类证据相互补充：理论证明支撑算法性质，有限对照用于核验当前实现，并不替代数学证明。

外层仍由 GCLS、有限 $\varepsilon$ 网格和迭代细化决定哪些 Offset 被访问，因而决定观测 Pareto 前沿的覆盖范围。第~\ref{subsec:exp-rq6} 节的种子、网格与搜索路径敏感性以及第~\ref{subsec:exp-rq5} 节的细化状态均表明，候选覆盖会随搜索配置变化；第~\ref{subsec:exp-rq3} 节的 better-feasible 对照进一步说明，部分小实例上的 GCLS best-found 仍存在改进空间。于是，当前分层结构可以准确概括为 exact inner 与 heuristic outer：内层最优值对已访问 Offset 是确定的，外层尚未访问的区域则可能带来新的非支配候选。图~\ref{fig:exp-joint-pareto} 因而采用“观测 Pareto 前沿”这一术语，其覆盖范围由当前 Peak 约束、候选域和搜索预算共同界定。

\subsection{CPU 开销代理的解释与平台标定}
\label{subsec:discussion-cpu-proxy}

CPU 开销代理指标用于比较不同 Offset 所诱导的软件周期组织相对代价。它保留 MainFunction 数量、组内报文数量、每组最大可行 TimeBase，以及固定调用成本与逐报文增量成本之间的比例，使调用频率和单次处理规模能够在统一模型中比较。其研究价值在于揭示 Offset 与软件周期入口之间的结构耦合；目标平台上的完整执行路径、调度抢占和微体系结构效应仍需通过实测建立映射。

参数 $\rho=C_0/C_1$ 表示每次 MainFunction 固定调用成本相对于单条报文增量成本的比例，而非普通的搜索超参数。第~\ref{subsubsec:exp-rq6-rho} 节的敏感性结果显示，$\rho$ 的变化可能改变 MainFunction 划分、$P^\star$ 以及观测 Pareto 结构。正式实验采用的 $\rho=1$ 提供了明确、可重复的未标定基准场景，足以用于研究 Offset 与软件周期组织之间是否存在结构性耦合；它尚不足以单独决定特定 ECU 上应采用哪个折中点。

从结构代理走向目标 ECU，需要在目标硬件与软件栈上对代表性 MainFunction 配置进行性能剖析，估计一次调用的固定成本 $C_0$ 和单条报文的平均增量成本 $C_1$。若实测成本随报文类型、处理路径或分组规模呈现明显差异，可在保持组成本可评价的前提下建立更细的执行成本模型。完成标定后，固定 Offset 下的分组评价和联合搜索框架仍可继续使用，变化的是组成本模型、参数解释及其与实际执行时间的对应关系。CPU 开销代理指标由此承担候选结构比较的角色，而实际 CPU 负载与可调度性结论应由目标平台测量和系统级时序分析给出。

\subsection{工程适用范围与部署路径}
\label{subsec:discussion-deployment}

九个真实 CAN FD 网段使本文能够在实际工程规模和通信矩阵上检验方法，相较纯合成实例更直接地反映了输入资格、周期组合和候选相位共同作用时的可运行性。与此同时，这些数据仍来自有限的项目环境，复杂路由、不同 Offset 来源语义以及其他软件执行架构尚未形成充分样本。因此，网段依赖性结论适用于当前数据所覆盖的工程条件，向其他车型、ECU、协议配置和网络架构推广时仍需补充跨项目验证。

本文直接优化的是 release shaping，即周期报文进入总线竞争之前的局部释放结构。减少高负载时隙中的同步释放能够改善竞争的输入条件，但真实 CAN/CAN FD 时序还由优先级仲裁、非抢占阻塞、控制器排队、其他流量以及错误重传共同决定。因而，降低释放热点改善的是总线竞争的输入条件，而不是直接给出响应时间或可调度性结论；附录第~\ref{subsec:appendix-conservative-service} 节的保守服务时间诊断也应在这一层次上解释。

当前工程工具已经将 CAN-only 与 CAN--CPU Joint 纳入同一面向实际工程输入的 GUI 工作流。CAN-only 路径继续完成输入资格筛选、Offset 优化、结果分析与审计以及 DBC Offset 配置导出；Joint 路径则在相同工程输入基础上执行联合搜索，展示以 $Q^{\mathrm{ss}}$ 和 CPU Cost Proxy 为坐标的观测 Pareto 前沿及自动推荐诊断，并生成 MainFunctionTx 分组及相应 TimeBase 建议。成功网段还可导出联合结果、观测前沿和 MainFunction 推荐产物，使论文中的联合模型进入了可交互、可审计的工程工作流。

Joint 的正式工程输出统一绑定核心算法给出的同一自动推荐分配。该分配同时驱动 Offset 表、负载曲线、热力图、DBC replacement、MainFunction 分组和联合导出，避免 GUI 展示、配置写回与软件侧建议采用不同候选。观测前沿只用于解释权衡，GUI 不重新计算推荐，也不允许将任意手选 Pareto 点作为正式结果；若核心算法没有返回合法推荐，工具保留联合诊断与前沿产物，但不伪造 Offset 分配、不生成 MainFunction 推荐，也不写回 DBC，以失败关闭（fail-closed）保持配置输出与模型语义一致。

该工具已经投入实际工程使用，现场应用观察到明显的周期释放热点改善。九网段锁定实验回答方法在受控真实输入上的可重复表现，工程部署则说明方法能够进入输入检查、结果审阅和配置输出流程；两者共同支持工程可用性，但属于不同证据来源，本文不将其合并统计。

当前能力已经从 CAN-only Offset 配置扩展到 Joint 推荐以及 MainFunctionTx 与 TimeBase 建议，但这些建议尚未自动物化为目标项目可直接使用的 AUTOSAR 软件配置。下一阶段需要把推荐映射到具体 AUTOSAR 配置与 ECU 构建流程，并在完成目标硬件性能剖析后开展总线与任务时序分析、硬件在环和整车回归。由此，本文已经形成工程建议与配置候选的闭环，而从推荐结果到可执行配置及系统级验证仍是后续部署路径。

\clearpage
\section{结论}
\label{sec:conclusion}

本文研究的核心问题是，同一 Offset 一方面改变 CAN 周期报文的释放相位，另一方面约束软件周期入口可采用的调用周期。围绕这一耦合关系，本文形成了 CAN 侧 Offset 均衡、固定 Offset 下 MainFunction 精确评价以及 CAN--CPU 联合权衡搜索的一体化方法。

\subsection{方法与理论结论}
\label{subsec:conclusion-properties}

在 CAN 侧，本文建立加权离散释放模型，以稳态峰值和 $Q^{\mathrm{ss}}$ 分别刻画最高局部聚集与全窗口均衡，并利用 GCLS 在有限 Offset 空间中搜索相位配置。这一建模将周期释放热点转化为可直接优化和比较的工程配置问题，同时保持 Offset 不改变周期报文长期平均释放工作量的基本性质；GCLS 承担外层启发式搜索。

在软件侧，周期与 Offset 共同决定周期入口可采用的调用周期，多报文共享入口后，该约束进一步作用于 MainFunction 分组及 TimeBase。基于 CPU 开销代理模型，相同 $D$ 类型压缩与确定性子集动态规划使固定完整 Offset 下的软件分组子问题能够精确求解，从而为每个已访问相位配置提供一致的软件侧最小代理成本。

在联合层，固定 Offset 的精确内层与 Peak 上界、$\varepsilon$-constraint、累计候选归档和迭代细化共同组织 CAN 均衡与软件成本之间的权衡，并以观测 Pareto 前沿及确定性后处理给出解释性推荐。该分层结构将固定 Offset 下的软件分组确定性与外层 Offset 搜索覆盖问题分离：软件分组属于精确子问题，Offset 外层仍采用启发式搜索。

\subsection{实验结果与工程应用}
\label{subsec:conclusion-results}

锁定的九网段 CAN FD 实验中，Peak 相对于 Original 在 6/9 个网段降低了稳态峰值，说明真实工程配置中存在可利用的相位重排空间。Balanced 在 9/9 个网段与 Peak 得到相同的核心 CAN 指标，而 Variance 在部分网段呈现 $Q^{\mathrm{ss}}$ 降低、峰值升高的结果，表明整体均衡与最高局部负载之间不存在一般的单调一致关系。

固定 Offset 下的 MainFunction 求解器与 141/141 组独立报文级穷举对照一致；九个真实网段的不同 $D$ 类型数为 4--6，当前实验平台上的冷启动求解均低于 $1\,\mathrm{ms}$。数学证明与独立实现对照共同支持固定 Offset 下的精确求解器，当前真实输入规模下的内层计算成本也较小。

联合实验在六个网段形成多点观测 Pareto 前沿，另三个网段为单点前沿，说明 CAN 均衡与软件调度代理成本之间的权衡具有明显网段依赖性。部分前沿及 Offset 分配对随机种子、$\rho$ 和搜索路径仍表现出敏感性，因此联合结果继续按“观测”口径解释。

本文方法已经形成面向实际工程输入的 GUI 工具，CAN-only 与 CAN--CPU Joint 均进入统一工程工作流。Joint GUI 能够执行联合搜索、展示观测 Pareto 前沿与核心自动推荐、生成 MainFunctionTx 与 TimeBase 建议，并以同一正式推荐驱动 Offset 展示、DBC 配置输出和联合审计产物。实际工程应用中已经观察到明显的周期释放热点改善；该现场观察与九网段受控实验承担不同证据角色，不作合并统计。

\subsection{研究局限与后续工作}
\label{subsec:conclusion-limitations}

现有方法仍受外层 Offset 搜索覆盖范围的限制，也缺少更强的解质量评价或界。后续可结合更广的组合邻域、小规模精确求解与 CP-SAT 对照以及可证明有界的方法，评估当前观测前沿的覆盖程度；对 $T_i>O^{\rm lim}$ 的长周期报文，也可采用独立且更稀疏的候选栅格扩展搜索域。

除搜索覆盖外，CPU 侧的结构代理还需要在目标 ECU 上完成成本标定。通过性能剖析估计 $C_0$、$C_1$ 与 $\rho$，并在需要时按报文类型、处理路径或分组规模扩展成本模型，可将当前结构代理映射为平台相关的软件执行成本评价。

在此基础上，工程部署还需从推荐结果进一步走向目标项目的可执行配置。当前 GUI 已经完成 Joint 搜索以及 MainFunctionTx 与 TimeBase 推荐和导出，后续需要将这些建议自动映射为可用的 AUTOSAR 配置并接入 ECU 构建和实际软件配置流程，再通过目标硬件性能剖析、CAN 与任务时序分析、硬件在环、整车回归和跨项目数据验证其系统级效果。
\clearpage
